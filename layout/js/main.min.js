...
ScrollReveal().reveal('.home .right *, .about .right *, .contact .right *', {
  delay: 100,
  origin: 'right',
  interval: 25,
});
...
// JSON fetch to portfolio slider
function portfolio_slider_JSON() {
  $.getJSON('projects.json', function (portfolio) {
    for (p = 0; p < portfolio.length; p++) {
      $('.portfolio_slider').append(
        `<div style="--bg: url('../../img/projects/${portfolio[p].bg}.webp')" data-index="${p}" id="project-pop-up">
          <h3>${portfolio[p].title}</h3>
        </div>`,
      );
    }
  }).done(function () {
...
$('.portfolio_slider').on('click', '#project-pop-up', (e) => {
  e.preventDefault();
  $(document.body).addClass('active');
  $('.portfolio-pop-up').addClass('show');
});

// close portfolio popup
$('.portfolio-pop-up .close').on('click', (e) => {
  e.preventDefault();
  $(document.body).removeClass('active');
  $('.portfolio-pop-up').removeClass('show');
});

// JSON fetch to portfolio popup
$('.portfolio_slider').on('click', '#project-pop-up', function (e) {
  e.preventDefault();
  $.getJSON('projects.json', function (project) {
    let p = $(e.target).data('index');
    $('.portfolio-pop-up').attr(
      'style',
      `--bg-pop: url('../../img/projects/${project[p].bg}.webp')`,
    );
    // define and append text inside Json file
    $('.portfolio-pop-up .image').html(
      `<img src="img/projects/${project[p].image}.webp"/>`,
    );
    $('.portfolio-pop-up .title').html(project[p].title);
    $('.portfolio-pop-up .description').html(project[p].desc);
    $('.portfolio-pop-up .link').html(
      `<a target="_blank" href="${project[p].link}">Visit website</a>`,
    );
    // define and append array inside Json file
    $('.portfolio-pop-up .skills').empty();
    jQuery.each(project[p].skills, function (s, val) {
      $('.portfolio-pop-up .skills').append('<li>' + val + '</li>');
    });
  });
});
...
window.addEventListener('click', (e) => {
  if (e.target.classList.contains('page-content')) {
    ...
    document.querySelector('.portfolio-pop-up').classList.remove('show');
  }
});
