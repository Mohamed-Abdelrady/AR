...
function contactJSON() {
  $.getJSON('contact.json', function (contact) {
    for (c = 0; c < contact.length; c++) {
      $('.contacts').append(
        `<a href="${contact[c].link}" target="_blank"><img src="img/icons/${contact[c].img}.svg" alt="${contact[c].img}" loading="lazy" width="35" height="35"/></a>`,
      );
    }
  });
}
    ...
    for (w = 0; w < tech.length; w++) {
      $('.technologies_slider').append(
        `<div class="technology">
          <img src="img/tech/${tech[w].icon}.svg" alt="${tech[w].name}" loading="lazy" width="24" height="24"/>
...
$('.projects-pop-up .close').on('click', (e) => {
  ...
  document.querySelector('.page-content').classList.remove('show');
});
...
$('.projects_slider').on('click', '#project-pop-up', function (e) {
  ...
  $.getJSON('projects.json', function (project) {
    ...
    if (project[p].link.length != 0) {
      $('.projects-pop-up .link').html(
        `<a target="_blank" href="${project[p].link}">Visit website</a>`,
      );
    } else {
      $('.projects-pop-up .link').html(
        `<a href="${project[p].link}" style="pointer-events: none;">Unknown link</a>`,
      );
    }
